...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found'); window.location.href = 'Qthree_reportingd.php';</script>";exit;
}
foreach($delete_ids as $key => $val)
 ...
 header("Location:Qthree_reportingd.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_pland` SET indicator='$txtindicatore[$i]',quarterthree_report='$txtquarterthree_report[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:Qthree_reportingd.php?msg=Update Successfully");  
}
...
<tr>
    <td>
<h3>TOTAL EDIT IDS "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = 'Qthree_reporting.php';</script>";}?>"</h3>
...
<form action="editquarter3d.php" method="post" autocomplete="off">
<?php
$j = 0;
...
<input type="text" name="txtquarterthree_report[]" value="<?=$row['quarterthree_report'];?>" class="form-control" id="quarterthree_report">
      </td>
    <td align="right">
