...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = 'Qtwo_reportingC.php';</script>";exit;
}
foreach($delete_ids as $key => $val)
 ...
 header("Location:Qtwo_reportingC.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_planC` SET indicator='$txtindicatore[$i]',quartertwo_report='$txtquartertwo_report[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:Qtwo_reportingC.php?msg=Update Successfully");  
}
...
<tr>
    <td>
<h3>TOTAL EDIT IDS "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = 'Qtwo_reportingC.php';</script>";}?>"</h3>
...
<form action="editquarter2C.php" method="post" autocomplete="off">
<?php
$j = 0;
...
<input type="text" name="txtquartertwo_report[]" value="<?=$row['quartertwo_report'];?>" class="form-control" id="quartertwo_report">
      </td>
    <td align="right">
