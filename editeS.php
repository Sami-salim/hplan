...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = 'planingS.php';</script>";exit;
}
foreach($delete_ids as $key => $val)
  {
  $delete_que="DELETE from `yearly_planS` WHERE `id`='$val'";
  $dbConnection->exec($delete_que);
  }
 header("Location:planingS.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_planS` SET indicator='$txtindicatore[$i]',unit='$txtunit[$i]',baseline='$txtbaseline[$i]',yeariyplan='$txtyeariyplan[$i]',quarterone='$txtquarterone[$i]',quartertwo='$txtquartertwo[$i]',quarterthree='$txtquarterthree[$i]',quarterfour='$txtquarterfour[$i]' WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:planingS.php?msg=Update Successfully");  
}
...
<form action="editeS.php" method="post" autocomplete="off">
<?php
$j = 0;
foreach($edit_ids as $key => $val)
  {
  $view_que="SELECT * FROM `yearly_planS` WHERE `id`='$val'";
