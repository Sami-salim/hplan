...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = 'Qone_reportingd.php';</script>";exit;
}
foreach($delete_ids as $key => $val)
 ...
 header("Location:Qone_reportingd.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_pland` SET indicator='$txtindicatore[$i]',quarterone_report='$txtquarterone_report[$i]',quarterone_achiv='$txtquarterone_achiv[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:Qone_reportingd.php?msg=Update Successfully");  
}
...
<tr>
    <td>
<h3>TOTAL EDIT IDS "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = 'Qone_reportingC.php';</script>";}?>"</h3>
...
<form action="editquarter1d.php" method="post" autocomplete="off">
<?php
$j = 0;
...
<input type="text" name="txtquarterone_report[]" value="<?=$row['quarterone_report'];?>" class="form-control" id="quarterone_report">
<input type="text" name="txtquarterone_achiv[]" value="<?=$row['quarterone_achiv'];?>" class="form-control" id="quarterone_achiv">
      </td>
    <td align="right">
