...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = 'Qone_reportingC.php';</script>";exit;
}
foreach($delete_ids as $key => $val)
 ...
 header("Location:Qone_reportingC.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_planC` SET indicator='$txtindicatore[$i]',quarterone_report='$txtquarterone_report[$i]',quarterone_achiv='$txtquarterone_achiv[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:Qone_reportingC.php?msg=Update Successfully");  
}
...
<form action="editquarter1C.php" method="post" autocomplete="off">
<?php
$j = 0;
