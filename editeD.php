...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_planD` SET indicator='$txtindicatore[$i]',unit='$txtunit[$i]',baseline='$txtbaseline[$i]',yeariyplan='$txtyeariyplan[$i]',quarterone='$txtquarterone[$i]',quartertwo='$txtquartertwo[$i]',quarterthree='$txtquarterthree[$i]',quarterfour='$txtquarterfour[$i]' WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
header("Location:planingD.php?msg=Update Successfully");  
}
...
<tr>
    <td>
<h3>TOTAL EDIT IDS "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = 'planingD.php';</script>";}?>"</h3>
...
<form action="editeD.php" method="post" autocomplete="off">
<?php
$j = 0;
